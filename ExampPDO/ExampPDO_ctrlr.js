...
i2b2.ExampPDO.Init = function(loadedDiv) {
	...
	ocpu.seturl(i2b2.ExampPDO.cfg.config.plugin.ocpuUrl);
	...
	i2b2.sdx.Master.AttachType("ExampPDO-ENSDROP", "ENS", op_trgt);
	...
	i2b2.sdx.Master.setHandlerCustom("ExampPDO-ENSDROP", "ENS", "DropHandler", i2b2.ExampPDO.ensDropped);
	...
	this.yuiTabs.on('activeTabChange', function(ev) { 
		...
		//	if (i2b2.ExampPDO.model.conceptRecord && i2b2.ExampPDO.model.prsRecord) {
			if (i2b2.ExampPDO.model.prsRecord) {
...
i2b2.ExampPDO.Unload = function() {
	...
	i2b2.ExampPDO.model.ensRecord = false;
	i2b2.ExampPDO.model.conceptRecord = false;
	i2b2.ExampPDO.model.dirtyResultsData = true;
	i2b2.ExampPDO.model.outputOptions.patients = true;
	i2b2.ExampPDO.model.outputOptions.events = false;
	i2b2.ExampPDO.model.outputOptions.observations = false;
	i2b2.ExampPDO.model.outputOptions.modifiers = false;
	i2b2.ExampPDO.model.outputOptions.observers = false;	
	return true;
};
...
i2b2.ExampPDO.ensDropped = function(sdxData) {
	sdxData = sdxData[0];	// only interested in first record
	// save the info to our local data model
	i2b2.ExampPDO.model.ensRecord = sdxData;
	// let the user know that the drop was successful by displaying the name of the patient set
	$("ExampPDO-ENSDROP").innerHTML = i2b2.h.Escape(sdxData.sdxInfo.sdxDisplayName);
	// temporarly change background color to give GUI feedback of a successful drop occuring
	$("ExampPDO-ENSDROP").style.background = "#CFB";
	setTimeout("$('ExampPDO-ENSDROP').style.background='#DEEBEF'", 250);	
	// optimization to prevent requerying the hive for new results if the input dataset has not changed
	i2b2.ExampPDO.model.dirtyResultsData = true;		
...
i2b2.ExampPDO.plot = function(data){
//$j("div[id='plotdiv']").rplot('I2B2Ocpu', {xml:data});
//$j("div[id='plotdiv']").rplot('testPlot', {symetric:'E'});
// ocpu.rpc("getDataTable",{
//    idResult:944 ,
 //   idProject:"_multi_pseudo" 
 //   ,     idSession: "yYNUuW2Omabh3mP9yDko"
//    ,     researchGoal: "Test"
//    ,  exportData: {test:"patient"}
//}, function(output){
...
//});
 ...
 ocpu.call("testTable",{
    ...
    alert("go to /ocpu/tmp/" + session.getKey()+ "/tar");
});
...
i2b2.ExampPDO.getResults = function() {
	...
	//		Element.select(divResults, '.InfoPDO-Response .originalXML')[0].innerHTML = '<pre>'+i2b2.h.Escape(results.msgResponse)+'</pre>';
			...
			i2b2.ExampPDO.plot(results.msgResponse);
		}
		...
		i2b2.CRC.ajax.getPDO_fromInputList("Plugin:ExampPDO", { PDO_Request: msg_filter}, scopedCallback);
